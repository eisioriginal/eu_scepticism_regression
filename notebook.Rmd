...
data_tokens_sentences2 <- 
  tokens_compound(data_tokens_sentences, pattern = dic_entry[dic_nwords2 == 2], concatenator = " ")
...
for(i in 1:(length(selected_sentences)-1))
{
  if(selected_sentences[i] && (i > 1))
  {
    selected_sentences[(i-1):(i+1)] <- T
  }
} 
...
data_dfm_entries_dict_sub <- 
  data_dfm_entries_dict_sub[rowSums(data_dfm_entries_dict_sub) >=10, ]
